...
test1:
	...
	@valgrind --leak-check=full ./exe/server ./tests/test1.conf > tests/output/server.log 2>tests/output/valgrind.log & echo "$$!" > tests/output/server.PID
	@sleep 2
	@bash tests/test1.sh
	@echo Sending SIGHUP to the server
	@kill -s 1 `cat tests/output/server.PID`
	@bash tests/wait `cat tests/output/server.PID`
	@rm tests/output/server.PID
	@echo Done
...
test2:
	...
	@valgrind --leak-check=full ./exe/server ./tests/test2.conf > tests/output/server.log 2>tests/output/valgrind.log & echo "$$!" > tests/output/server.PID
	@sleep 2
	@bash tests/test2.sh
	@sleep 5
	@echo Sending SIGHUP to the server
